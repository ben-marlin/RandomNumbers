...
but that seems bulky. It does, however, let us introduce a little vocabulary. The first line of that block is a *declaration* - it declares that *x* is an integer. The second line is an *assignment*. We may also call this process *instantiation*, as it creates an instance of an integer. Notice that an assignment uses the = operator. Later, you'll use == to compare and see whether things are equal.

Quick note: variable names have to start with a letter and can include numerals, or _ in them. Avoid other special characters.
...
Now, create an instance of the random number generator and change your variable assignment like so. Place this inside the main method.
...
The second line contains rand.nextInt() - which asks the object named rand to get an integer from a huge list it has available. But we didn't want just any integer, we wanted something from 1 to 6, so we used rand.nextInt(1,6). If we had only said rand.nextInt(6), it might have chosen a 0 and definitely would not choose 6. VSCode should supply the words origin and bound to let you know their purposes.
...
Next, make a line to print `"integer sum is " + firstInt + secondInt`. Run it. What's wrong? Insert a comment line to explain your observation.
